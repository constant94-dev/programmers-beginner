...
// 위와 같은 코드를 사용했다... 하지만, 이렇게 했을 때 덮어씌워지지 않고 해당 인덱스 값이 한 칸 밀려서 저장되는 이슈가 있었다... 이걸 해결하기 위해서는 해당 인덱스 값을 삭제하고 값을 추가하는 코드가 필요했다... 이렇게 하니 코드가 너무 복잡해져 좋은 아이디어가 아니라고 생각해 답을 찾아보았다...
```
...
<h4>
Day24_3 문제 풀이 중 문자열 두개를 비교하여 순서만 바꿨을 때 똑같은 문자열을 만들 수 있으면 결과 1을 만들어내야 한다... `replace()`함수는 해당하는 문자를 문자열에서 모두 지우기 때문에 사용할 수 없고, `substring()`함수를 사용해서 제출하니 23개 케이스 중 8개에서 오류가 발생했다... 입력값 노가다 작업을 통해 엣지케이스를 찾아냈다...
</h4>
