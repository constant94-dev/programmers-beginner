...
<h4>
Day22_4 문제 풀이 중 테스트 케이스는 통과했지만 제출 했을 때 48개 케이스중 4개 오류가 발생해 분석해보았다... 2개 오류는 결과값이 소수점 아래가 없는 정수로만 이루어졌을 때 처리가 안되어 있어 결과 값이 정수일 때 처리를 해주니 해결되었다... 나머지 2개의 오류는 정말 찾기 힘들었다.... 답을 보고싶었지만 48개 케이스 중 46개는 맞았던지라 끝까지 내 힘으로 부딪혀보고 싶었다...
최소,최대,중간 값을 모두 해보았지만 찾을 수 없어 길고 긴 입력값 노가다를 시작했다...
</h4>

<h4>
길고 긴 입력값 노가다 작업을 통해 아래와 같은 문제를 발견했다...
</h4>

<h4>
결론은, 내 로직에서 결과값을 만들어낼 때 소인수분해한 값들을 리스트에 저장하고 리스트에 2또는5만 존재했을 때 '1'을 반환 아닐 때 '2'를 반환한다.... 
예를들어, {2,2,5} 배열은 '1'을 반환 / {2,3,5} 배열은 '2'를 반환 But 2또는5가 아닌 값이 발견된 순간 반복문을 멈추고 결과를 리턴해야하는데 나는 배열의 끝까지 탐색을 한 후 결과를 리턴하게되어 '2'를 반환해야할 때, '1'을 반환하는 문제가 발견되었고 해당 코드를 수정하니 해결되었다....

아래 제시된 이미지가 수많은 테스트 케이스를 지우고 작성하고를 반복해서 찾아낸 엣지케이스이다.
</h4>

![feature](images/Day22_4.png)

</p>

<p>
<h4>[23.10.15]<br/>
: 프로그래머스 Day23 문제 풀이 및 오류케이스 분석
</h4>

<h4>
Day23_1 문제 풀이 중 첫 접근은 정수 배열 요소들을 'n'과 가까운 수부터 정렬하는것이다. 작은 수부터  차근차근 비교하기 위해 정수 배열을 오름차순 정렬하였다... 그 이후에 이중 반복문을 돌며 하나의 인덱스를 정수 배열의 모든 요소를 비교해 가까운수를 앞으로 보내려고 했다...
생각은 좋았지만 구현을 할 때 머리가 막혀서 시간을 많이 잡아먹었다... 결국, 답을 찾아보았고 다른 사람들도 나와 비슷하게 풀이를 작성했다... 하지만, 반복문에서 어떻게 비교를 시작할지 몰랐는데 임시변수를 하나 만들고 현재 요소가 'n'과 가까운 수라면 교체를 수행한다... 단순하지만 머리가 멍해져 답을보고 이해가 되는 문제가 되었다...
</h4>
</p>
...
<h4>
Day23_2 문제 풀이 중 입력된 점수의 평균을 구한 후 등수를 매기고 결과를 리턴하는데 핵심 아이디어는 인덱스별로 비교가 끝났을 때 순위를 주는것이다....
그런다음 다음번 순위 판별을 위해 순위를 '1'로 초기화 해서 전체 평균점수에 대한 순위를 매길 수 있다... 하지만, 코드를 제출해보니 12개 케이스 중 2개가 오류가 난다... 이유를 찾기 위해 입력값 노가다를 시작... 최소,최대,중간 값 대입을 해보니 문제를 확인할 수 있었다...
</h4>

<h4>
문제는 모든 평균점수가 `int`형태로 값을 저장하다보니 소수점 자리 비교가 필요한 평균점수들이 동률이 나오는 문제가 발생했다...
예를들어,

```java
{{0, 0}, {100, 100}, {1, 100}, {100, 1}, {50, 50}, {1, 50}, {50, 1}, {1, 99}, {99, 1}, {100, 50}}
// 코드를 수정하기 전에는 {10,1,3,3,3,8,8,3,3,2}가 결과로 나왔다...
// 올바른 순위는 {10,1,3,3,5,8,8,5,5,2}가 나와야한다...
```

그래서 평균값을 저장하는 변수의 자료형을 'double'로 변경해주니 해결되었다...
</h4>

<h4>
Day23_3 문제 풀이 중 발음할 수 있는 단어를 조합해야 된다고 생각했다... 왜냐하면 문제에서 제시된 입출력 예시를 확인해보니 입력으로 주어진 문자열이 서로가 붙어서 문자열을 이루더라도 발음할 수 있는 단어가 되기 때문이다... 그래서 총 4개의 문자열을 2개씩, 3개씩, 4개씩 조합해서 입력값과 비교해 같을 때마다 결과를 1증가 해주는 코드를 작성했다... 이렇게 코드를 만들고 제출하니 한번에 통과했다... 하지만, 다른 사람들은 어떻게 작성했는지 궁금해 찾아보았다...
</h4>

<h4>
역시나 내가 만든 코드보다 더 간결하게 답을 만들어낼 수 있었다...
이 문제는 replace 관련 메서드를 활용하여 많이 풀이 하는걸로 확인했다...
내가 찾고자하는 문자열을 일괄적으로 변경하는것이다... `aya -> 0` 이런 식이다...
문자열 관련 문제를 풀 때는 'replace()' 함수를 활용하는쪽으로 아이디어를 생각해봐야겠다...
</h4>
</p>

<p>
<h4>[23.10.19]<br/>
: 프로그래머스 Day24 문제 풀이 및 오류케이스 분석
</h4>

<h4>
Day24_2 문제 풀이 중 첫 접근은 입력으로 주어진 문자열을 십진수 형태로 더한다... 그러면 값은 `0,1,2,3`으로 제한된다... 그래서 십진수 형태의 값을 리스트에 저장해 하나씩 가져와서
...
// 위와 같은 코드를 사용했다... 하지만, 이렇게 했을 때 덮어씌워지지 않고 해당 인덱스 값이 한 칸 밀려서 저장되는 이슈가 있었다...
// 이걸 해결하기 위해서는 해당 인덱스 값을 삭제하고 값을 추가하는 코드가 필요했다...
// 이렇게 하니 코드가 너무 복잡해져 좋은 아이디어가 아니라고 생각해 답을 찾아보았다...
```

</h4>

<h4>
역시나 답은 간결했다... `Integer.parseInt(String s, int i)`함수를 제공해주는데 파라미터 s 문자열의 진수를 지정해 10진수로 변환하는 것이다... 그리고 여기서 만들어진 10진수를 `toBinaryString(int i)`를 사용하면 10진수를 2진수로 변경해준다... 이렇게 코드를 변경하니 해결이되었다... 내장된 함수에서 이렇게 편리한 방법을 제공해주는줄 모르고 접근하니 내가 `toBinaryString()` 함수를 만들어야하는 어려움이 생긴다...
</h4>

<h4>
Day24_3 문제 풀이 중 문자열 두개를 비교하여 순서만 바꿨을 때 똑같은 문자열을 만들 수 있으면 결과 1을 만들어내야 한다... `여기서 핵심은 문자하나당 하나의 문자만 매칭시켜야 한다!!!!`....  `replace()`함수는 해당하는 문자를 문자열에서 모두 지우기 때문에 사용할 수 없고, `substring()`함수를 사용해서 제출하니 23개 케이스 중 8개에서 오류가 발생했다... 입력값 노가다 작업을 통해 엣지케이스를 찾아냈다...
</h4>

![feature](images/Day24_3.png)

<h4>
위 그림이 찾아낸 엣지 케이스이다... 문제는 `substring()`함수를 사용할 때 문자 하나당 하나씩 매칭시켜 삭제하고 다음 문자를 가져와 비교해야 하는데 이중 반복문 구조이다보니 하나의 문자를 전체 문자열을 순회하며 삭제하고 있었던 것이다... 간단히 말해서 `replace()`함수를 사용할 때와 같은 결과를 보여주기 때문에 `substring()`함수를 사용해 문자를 하나 제거하면 반복문을 멈추고 다음 반복을 진행해야한다... 이렇게 코드를 수정하니 해결되었다...
</h4>
</p>

<p>
<h4>[23.10.23]<br/>
: 프로그래머스 Day25_1 문제 풀이
</h4>
</p>

<p>
<h4>[23.10.24]<br/>
: 프로그래머스 Day25_2 문제 풀이
</h4>
</p>

<p>
<h4>[23.10.26]<br/>
: 프로그래머스 Day25_3 문제 풀이
</h4>
</p>

<p>
<h4>[23.10.27]<br/>
: 프로그래머스 Day25_4 문제 풀이
</h4>
</p>