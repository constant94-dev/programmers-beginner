...
1. `substring()` 함수는 변경하려는 문자 이전 값과 이후 값을 조합해 사용한다.
2. `toCharArray()` 함수는 변경하려는 문자 인덱스만 알면 해당 인덱스의 대입만 하면된다.
3. `StringBuilder()` 클래스는 `setCharAt()` 함수를 사용해 변경하려는 인덱스 번호와 문자를 사용하면 된다. 
