class Solution {
    public int[] recur(int[] answer, int[] emergency, int maxValue, int emergencyPriority, int emergency_length){
        ...
        // 3. 해당 인덱스 번호를 결과 배열 인덱스의 번호를 매긴다.
        for (int i=0; i<emergency_length; i++){
        ...
        emergency[currentMax_idx] = 0; // 현재 가장높은 응급도 인덱스의 값을 '0' 변경
        answer[currentMax_idx] = emergencyPriority; // 현재 응급도를 결과 배열 인덱스 번호의 저장
